...
import { generateEinsatzbericht } from '../pdf-generator.js';
...
export async function renderIncidents() {
  ...
  _ffName = settings?.ff_name || 'FeuerwehrHub';
...
async function _loadStats() {
  try {
    ...
    const csvUrl = api.exportIncidentStats(_filters.year, 'csv');
    const pdfUrl = api.exportIncidentStats(_filters.year, 'pdf');
    row.innerHTML =
      ...
      `<div style="display:flex;align-items:center;gap:6px;margin-left:auto">
         <a href="${csvUrl}" download class="btn btn--sm btn--outline">CSV</a>
         <a href="${pdfUrl}" download class="btn btn--sm btn--outline">PDF</a>
       </div>`;
  } catch (_) {}
}
          ...
          ${items.map(r => `
            <tr>
              ...
              <td>
                ...
                ${r.source && r.source !== 'manual' ? `<span style="margin-left:4px;padding:2px 6px;border-radius:12px;font-size:10px;font-weight:700;background:rgba(88,166,255,0.15);color:#58a6ff;text-transform:uppercase">${esc(r.source)}</span>` : ''}
              </td>
              <td>
                <div class="btn-group">
...
async function _openModal(id, viewOnly) {
  ...
  try {
    ...
    loadVehiclesTab(id, true);
    loadPersonnelTab(id, true);
    ...
    footer.innerHTML = `
      ...
      <button class="btn btn--outline" id="btn-modal-pdf">PDF</button>
    ...
    document.getElementById('btn-modal-pdf').addEventListener('click', async () => {
      try {
        const res = await fetch(`/api/einsatzberichte/${id}/pdf`, {
        ...
        a.href     = url;
        a.download = `Einsatzbericht_${r.incident_number ?? id}.pdf`;
        a.click();
