...
function _renderPersonnelTab(wrap, incidentId, rows, memberOptions, readonly) {
      ...
      <table class="data-table">
        <thead>
          <tr>
            ...
            <th>Einsatz von</th><th>Einsatz bis</th>
            ${!readonly ? '<th></th>' : ''}
          </tr>
        </thead>
        <tbody>
          ${rows.map(r => `
            <tr data-pid="${r.id}">
              ...
              ${!readonly ? `
                <td>
                  <button class="btn btn--outline btn--sm" data-action="edit-p"
                    data-id="${r.id}"
                    data-name="${esc(r.display_name)}"
                    data-role="${esc(r.role_name || '')}"
                    data-function="${esc(r.function || '')}"
                    data-entry="${r.entry_time ? r.entry_time.slice(0,5) : ''}"
                    data-exit="${r.exit_time  ? r.exit_time.slice(0,5)  : ''}">✏</button>
  ...
  const addFormHTML = readonly ? '' : `
    <div id="p-add-form" style="display:none;background:var(--bg-card);border:1px solid var(--border);border-radius:10px;padding:16px;margin-bottom:12px">
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:10px">
        ...
        <div class="form-group">
          <label style="font-size:12px">Einsatz von</label>
          <input type="time" id="p-entry" />
        </div>
        <div class="form-group">
          <label style="font-size:12px">Einsatz bis</label>
          <input type="time" id="p-exit" />
        </div>
      </div>
      <div style="display:flex;gap:8px">
  ...
  document.getElementById('p-btn-submit')?.addEventListener('click', async () => {
    ...
    const body = {
      ...
      entry_time:   document.getElementById('p-entry')?.value || null,
      exit_time:    document.getElementById('p-exit')?.value || null,
    };
    try {
  ...
  // Edit-Button → Inline-Editform in der Zeile
  wrap.querySelectorAll('[data-action="edit-p"]').forEach(btn => {
    btn.addEventListener('click', () => {
      const pid = btn.dataset.id;
      const tr = btn.closest('tr');
      if (tr.querySelector('.p-edit-form')) return;

      const editRow = document.createElement('tr');
      editRow.innerHTML = `
        <td colspan="6" style="padding:10px;background:var(--bg-card);border:1px solid var(--border);border-radius:8px">
          <div style="display:grid;grid-template-columns:1fr 1fr 1fr 1fr 1fr;gap:8px;align-items:end">
            <div class="form-group" style="margin:0">
              ...
              <input type="text" id="ep-name-${pid}" value="${esc(btn.dataset.name)}" />
            </div>
            <div class="form-group" style="margin:0">
              <label style="font-size:11px">Dienstgrad / Rolle</label>
              <input type="text" id="ep-role-${pid}" value="${esc(btn.dataset.role)}" />
            </div>
            <div class="form-group" style="margin:0">
              <label style="font-size:11px">Funktion</label>
              <input type="text" id="ep-func-${pid}" value="${esc(btn.dataset.function)}" />
            </div>
            <div class="form-group" style="margin:0">
              <label style="font-size:11px">Einsatz von</label>
              <input type="time" id="ep-entry-${pid}" value="${esc(btn.dataset.entry)}" />
            </div>
            <div class="form-group" style="margin:0">
              <label style="font-size:11px">Einsatz bis</label>
              <input type="time" id="ep-exit-${pid}" value="${esc(btn.dataset.exit)}" />
            </div>
          </div>
          <div style="display:flex;gap:8px;margin-top:8px">
            <button class="btn btn--primary btn--sm" id="ep-save-${pid}">Speichern</button>
            <button class="btn btn--outline btn--sm" id="ep-cancel-${pid}">Abbrechen</button>
          </div>
        </td>`;
      editRow.classList.add('p-edit-form');
      tr.after(editRow);

      document.getElementById(`ep-cancel-${pid}`)?.addEventListener('click', () => editRow.remove());

      document.getElementById(`ep-save-${pid}`)?.addEventListener('click', async () => {
        const name = document.getElementById(`ep-name-${pid}`)?.value.trim();
        if (!name) { toast('Name eingeben', 'error'); return; }
        const body = {
          display_name: name,
          role_name:    document.getElementById(`ep-role-${pid}`)?.value.trim() || null,
          function:     document.getElementById(`ep-func-${pid}`)?.value.trim() || null,
          entry_time:   document.getElementById(`ep-entry-${pid}`)?.value || null,
          exit_time:    document.getElementById(`ep-exit-${pid}`)?.value || null,
        };
        try {
          await api.updateIncidentPersonnel(incidentId, pid, body);
          toast('Gespeichert');
          loadPersonnelTab(incidentId, false);
        } catch (e) { toast(e.message, 'error'); }
      });
