...
function openMitgliedModal(m = null) {
    ...
    <div class="modal">
      ...
      <div class="modal__body">
        <div class="form-grid">
          ...
          <div class="form-group">
            <label>Pause von <span class="text-muted">(für Jubiläen)</span></label>
            <input type="date" id="mm-pause-von" value="${m?.pause_von || ''}" />
          </div>
          <div class="form-group">
            <label>Pause bis</label>
            <input type="date" id="mm-pause-bis" value="${m?.pause_bis || ''}" />
          </div>
          <div class="form-group form-group--full">
  ...
  document.getElementById('save-mm-btn').addEventListener('click', async () => {
    const body = {
      ...
      pause_von:           document.getElementById('mm-pause-von').value || null,
      pause_bis:           document.getElementById('mm-pause-bis').value || null,
    };
    if (m) {
...
async function openEventDetail(id, isAdmin) {
  ...
  // Mitgliederliste nur laden, wenn Admin (für die Teilnehmer-Erfassung)
  let mitglieder = [];
  if (isAdmin) {
    try { mitglieder = (await api.getMitglieder() || []).filter(m => !m.archiviert); } catch (_) {}
  }
  const antwortByMitglied = Object.fromEntries(antworten.map(a => [a.mitglied_id, a]));
  ...
  const badgeFor = (a) => a
    ? `<span class="badge ${a === 'ja' ? 'badge--green' : a === 'nein' ? 'badge--red' : 'badge--gray'}">${a === 'ja' ? 'Ja' : a === 'nein' ? 'Nein' : 'Vielleicht'}</span>`
    : `<span class="text-muted">—</span>`;

  // Admin: pro Mitglied Ja/Nein/Vllt-Buttons (wie bei der Anwesenheit)
  const adminRows = mitglieder.map(m => {
    const cur = antwortByMitglied[m.id]?.antwort || null;
    return `
      <tr data-member-row="${m.id}">
        <td>${esc(m.vorname)} ${esc(m.nachname)}</td>
        <td data-current>${badgeFor(cur)}</td>
        <td style="white-space:nowrap">
          <button class="btn btn--xs btn--success ${cur==='ja'?'btn--active':''}"    data-mark="ja"         data-mid="${m.id}">Ja</button>
          <button class="btn btn--xs btn--danger  ${cur==='nein'?'btn--active':''}"  data-mark="nein"       data-mid="${m.id}">Nein</button>
          <button class="btn btn--xs btn--secondary ${cur==='vielleicht'?'btn--active':''}" data-mark="vielleicht" data-mid="${m.id}">Vllt.</button>
        </td>
      </tr>`;
  }).join('');

  // Nicht-Admin: nur Read-only Liste der bereits erfassten Antworten
  const readonlyRows = antworten.map(a => `
    <tr>
      <td>${esc(a.mitglied_name)}</td>
      <td>${badgeFor(a.antwort)}</td>
  ...
  modal.innerHTML = `
    <div class="modal modal--lg">
      ...
      <div class="modal__body">
        ...
        ${isAdmin ? `
          <h4 style="margin:1.5rem 0 .5rem">Teilnehmer erfassen</h4>
          <p class="text-muted text-sm" style="margin-top:0">Die Teilnahme wird zentral durch Berechtigte eingetragen.</p>
          ${mitglieder.length === 0 ? '<p class="empty-state">Keine Mitglieder vorhanden</p>' : `
          <div class="table-wrapper">
            <table class="data-table">
              <thead><tr><th>Mitglied</th><th>Status</th><th>Erfassen</th></tr></thead>
        ...
        ` : `
          <h4 style="margin:1.5rem 0 .5rem">Teilnehmer (${antworten.length})</h4>
          <p class="text-muted text-sm" style="margin-top:0">Die Teilnahme wird zentral durch Berechtigte eingetragen.</p>
          ${antworten.length === 0 ? '<p class="empty-state">Noch keine Teilnehmer erfasst</p>' : `
          <div class="table-wrapper">
            <table class="data-table">
              <thead><tr><th>Mitglied</th><th>Status</th><th>Kommentar</th></tr></thead>
  ...
  if (isAdmin) {
    modal.querySelectorAll('[data-mark]').forEach(btn => {
      btn.addEventListener('click', async () => {
        ...
        const row    = modal.querySelector(`[data-member-row="${mid}"]`);
        try {
          await api.setAntwortAdmin(id, mid, { antwort: mark, kommentar: null });
          row.querySelectorAll('[data-mark]').forEach(b => b.classList.remove('btn--active'));
          btn.classList.add('btn--active');
          row.querySelector('[data-current]').innerHTML = badgeFor(mark);
        } catch (e) { toast(e.message, 'error'); }
      });
    });
  }
...
async function openProtokollDetail(id, isAdmin) {
  ...
  modal.className = 'modal-overlay active';
  modal.innerHTML = `
    <div class="modal modal--xl" id="proto-print-area">
    ...
    el.innerHTML = `
      ...
      <div class="stats-row" style="margin-bottom:1.5rem">
        ...
        <div class="stat-card">
          ...
          <div class="stat-card__value" style="color:${summary.saldo >= 0 ? 'var(--gruen)' : 'var(--error)'}">${fmtEuro(summary.saldo)}</div>
        </div>
      </div>
