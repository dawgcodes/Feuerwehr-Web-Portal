            ...
            Some("file") => {
                ...
                if !ALLOWED_MIMES.contains(&mime.as_str()) {
                    return Err(AppError::BadRequest(
                        format!("Dateityp '{}' nicht erlaubt. Erlaubt: PDF, Word, Excel, ODF, Bilder, Textdateien", mime)
                    ));
                }
...
pub struct Mitglied {
    ...
    pub pause_von:           Option<NaiveDate>,
    pub pause_bis:           Option<NaiveDate>,
    pub archiviert:          bool,
    pub created_at:          DateTime<Utc>,
}
...
pub async fn list_mitglieder(
    ...
    let rows = sqlx::query_as::<_, Mitglied>(
        "SELECT id, mitgliedsnummer, vorname, nachname, email, telefon, geburtsdatum,
                ...
                fuehrerschein, pause_von, pause_bis, archiviert, created_at
...
pub async fn update_mitglied(
    ...
    let row = sqlx::query_as::<_, Mitglied>(
        "UPDATE verein_mitglieder SET
            ...
            pause_von         = $16,
            pause_bis         = $17,
            archiviert        = COALESCE($18, archiviert)
         WHERE id = $19
         RETURNING id, mitgliedsnummer, vorname, nachname, email, telefon, geburtsdatum,
                   eintrittsdatum, status, user_id, austritt_datum, austritt_grund,
                   bemerkung, kleidung_oberteil, kleidung_hose, kleidung_schuhe,
...
pub async fn get_ehrungen(
    ...
    // Dienstjahre abzüglich Pause (in ganzen Jahren, abgerundet)
    let jubilare = sqlx::query_as::<_, Jubilar>(
        "SELECT
            id,
            vorname || ' ' || nachname AS name,
            eintrittsdatum,
            (EXTRACT(YEAR FROM AGE(NOW(), eintrittsdatum))::int
                - COALESCE(CASE WHEN pause_von IS NULL THEN 0 ELSE ROUND((COALESCE(pause_bis, CURRENT_DATE) - pause_von)::numeric / 365.25)::int END, 0)
            ) AS dienstjahre,
            CASE
                WHEN (EXTRACT(YEAR FROM AGE(NOW(), eintrittsdatum))::int - COALESCE(CASE WHEN pause_von IS NULL THEN 0 ELSE ROUND((COALESCE(pause_bis, CURRENT_DATE) - pause_von)::numeric / 365.25)::int END, 0)) < 10 THEN 10
                WHEN (EXTRACT(YEAR FROM AGE(NOW(), eintrittsdatum))::int - COALESCE(CASE WHEN pause_von IS NULL THEN 0 ELSE ROUND((COALESCE(pause_bis, CURRENT_DATE) - pause_von)::numeric / 365.25)::int END, 0)) < 25 THEN 25
                WHEN (EXTRACT(YEAR FROM AGE(NOW(), eintrittsdatum))::int - COALESCE(CASE WHEN pause_von IS NULL THEN 0 ELSE ROUND((COALESCE(pause_bis, CURRENT_DATE) - pause_von)::numeric / 365.25)::int END, 0)) < 40 THEN 40
                WHEN (EXTRACT(YEAR FROM AGE(NOW(), eintrittsdatum))::int - COALESCE(CASE WHEN pause_von IS NULL THEN 0 ELSE ROUND((COALESCE(pause_bis, CURRENT_DATE) - pause_von)::numeric / 365.25)::int END, 0)) < 50 THEN 50
                ELSE 0
            END AS naechstes_jubilaeum
         FROM verein_mitglieder
         WHERE archiviert = FALSE
           AND status IN ('aktiv','ehren')
           AND (EXTRACT(YEAR FROM AGE(NOW(), eintrittsdatum))::int - COALESCE(CASE WHEN pause_von IS NULL THEN 0 ELSE ROUND((COALESCE(pause_bis, CURRENT_DATE) - pause_von)::numeric / 365.25)::int END, 0)) >= 9
           AND (EXTRACT(YEAR FROM AGE(NOW(), eintrittsdatum))::int - COALESCE(CASE WHEN pause_von IS NULL THEN 0 ELSE ROUND((COALESCE(pause_bis, CURRENT_DATE) - pause_von)::numeric / 365.25)::int END, 0)) < 50
...
pub async fn export_event_csv(
    ...
    for a in &antworten {
        ...
        csv.push_str(&format!("{};{};{}\n",
            csv_safe(&a.mitglied_name),
            antwort_de,
            csv_safe(a.kommentar.as_deref().unwrap_or(""))
        ));
    }
...
pub fn router(state: AppState) -> Router<AppState> {
    let protected = Router::new()
        ...
        .route("/logo",            post(upload_logo.layer(DefaultBodyLimit::max(5 * 1024 * 1024))).delete(delete_logo))
        // Schwarzes Brett
        .route("/posts",           post(create_post))
        .route("/posts/:id",       put(update_post).delete(delete_post))
        // Dokumente
        .route("/dokumente",       post(upload_document.layer(DefaultBodyLimit::max(20 * 1024 * 1024))))
