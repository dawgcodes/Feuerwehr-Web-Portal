...
pub async fn export_stats(
    ...
    // Fetch summary stats (same queries as get_stats)
    let total: i64 = sqlx::query_scalar(
        "SELECT COUNT(*) FROM incident_reports WHERE EXTRACT(YEAR FROM incident_date) = $1"
    ).bind(year).fetch_one(&state.db).await?;
    ...
    // Per-month breakdown
    #[derive(sqlx::FromRow)]
    struct MonthRow { month: i32, count: i64, category: Option<String> }
    let month_rows = sqlx::query_as::<_, MonthRow>(
        "SELECT EXTRACT(MONTH FROM ir.incident_date)::int AS month,
                COALESCE(it.category, 'sonstiges') AS category,
                COUNT(*) AS count
         FROM incident_reports ir
    ...
    ).bind(year).fetch_all(&state.db).await?;

    // Flatten to per-month totals for CSV
    let mut monthly: Vec<(i32, i64, i64, i64, i64)> = (1..=12).map(|m| (m, 0i64, 0i64, 0i64, 0i64)).collect();
    for r in &month_rows {
        if let Some(e) = monthly.get_mut((r.month - 1) as usize) {
            match r.category.as_deref() {
                Some("brand")     => e.1 += r.count,
                Some("thl")       => e.2 += r.count,
                Some("fehlalarm") => e.3 += r.count,
                _                 => e.4 += r.count,
            }
        }
    }
    ...
    match format {
        "pdf" => {
            ...
            let rows: Vec<Vec<String>> = monthly.iter().map(|(m, b, t, f, s)| {
                let total_m = b + t + f + s;
                vec![
                    month_names[(*m - 1) as usize].to_string(),
                    b.to_string(), t.to_string(), f.to_string(), s.to_string(),
                    total_m.to_string(),
                ]
            }).collect();

            let pdf_bytes = crate::pdf::PdfBuilder::new(format!("Einsatzstatistik {year}"))
                .heading(&format!("Einsatzstatistik {year}"))
                .separator()
                .key_value("Gesamt",    &total.to_string())
                .key_value("Brand",     &brand.to_string())
                .key_value("THL",       &thl.to_string())
                .key_value("Fehlalarm", &fehlalarm.to_string())
                .key_value("Sonstiges", &sonstiges.to_string())
                .spacer(5.0)
                .table(
                    vec!["Monat","Brand","THL","Fehlalarm","Sonstiges","Gesamt"],
                    rows,
                    vec![30.0, 28.0, 28.0, 34.0, 34.0, 26.0],
                )
                .build(&font_bytes)
                .map_err(|e| AppError::Internal(anyhow::anyhow!("{e}")))?;

            let filename = format!("Einsatzstatistik_{year}.pdf");
            Ok((
                [
        ...
        _ => {
            // CSV
            let month_names = ["Januar","Februar","März","April","Mai","Juni",
                               "Juli","August","September","Oktober","November","Dezember"];
            let mut csv = format!("Monat,Brand,THL,Fehlalarm,Sonstiges,Gesamt\n");
            for (m, b, t, f, s) in &monthly {
                let total_m = b + t + f + s;
                csv.push_str(&format!("{},{},{},{},{},{}\n",
                    month_names[(*m - 1) as usize], b, t, f, s, total_m));
            }
            csv.push_str(&format!("Gesamt,{},{},{},{},{}\n", brand, thl, fehlalarm, sonstiges, total));

            let filename = format!("Einsatzstatistik_{year}.csv");
            Ok((
                [
...
pub struct IncidentPersonnel {
    ...
    pub entry_time:   Option<chrono::NaiveTime>,
    pub exit_time:    Option<chrono::NaiveTime>,
    pub created_at:   chrono::DateTime<Utc>,
}
...
pub async fn add_incident_personnel(
    ...
    let row = sqlx::query_as::<_, IncidentPersonnel>(
        "INSERT INTO incident_personnel (incident_id, user_id, display_name, role_name, function, entry_time, exit_time)
    ...
    .bind(parse_time(body.entry_time.as_deref()))
    .bind(parse_time(body.exit_time.as_deref()))
    .fetch_one(&state.db)
    .await?;
    Ok(Json(row))
}

pub async fn update_incident_personnel(
    State(state): State<AppState>,
    Extension(claims): Extension<Claims>,
    ...
    let row = sqlx::query_as::<_, IncidentPersonnel>(
        "UPDATE incident_personnel
         SET display_name=$1, role_name=$2, function=$3, entry_time=$4, exit_time=$5
         WHERE id=$6 AND incident_id=$7 RETURNING *"
    )
    .bind(&name)
    .bind(body.role_name.as_deref())
    .bind(body.function.as_deref())
...
pub async fn download_attachment(
    ...
    let safe_name = attachment.filename.replace('"', "'").replace(['\r', '\n'], "");
...
pub async fn export_incident_pdf(
    ...
    ensure_incident_readable(db, id, &claims).await?;

    let report = sqlx::query_as::<_, IncidentReport>("SELECT * FROM incident_reports WHERE id = $1")
        .bind(id)
        .fetch_optional(db)
        .await?
        .ok_or(AppError::NotFound)?;

    let personnel = sqlx::query_as::<_, IncidentPersonnel>(
        "SELECT * FROM incident_personnel WHERE incident_id = $1 ORDER BY display_name",
    )
    .bind(id)
    .fetch_all(db)
    .await?;

    let vehicles = sqlx::query_as::<_, IncidentVehicle>(
        "SELECT * FROM incident_vehicles WHERE incident_id = $1 ORDER BY vehicle_name",
    )
    .bind(id)
    .fetch_all(db)
    .await?;
    ...
    let fmt_time = |t: Option<chrono::NaiveTime>| -> String {
        t.map(|t| t.format("%H:%M").to_string()).unwrap_or_default()
    };
    let fmt_opt = |s: Option<&str>| s.unwrap_or("-").to_string();

    let doc_title = format!(
        "Einsatzbericht {}",
        report.incident_number.as_deref().unwrap_or("(ohne Nr.)")
    );

    let mut builder = crate::pdf::PdfBuilder::new(&doc_title)
        .heading("Einsatzgrunddaten")
        .key_value("Einsatznummer", report.incident_number.as_deref().unwrap_or("-"))
        .key_value("Datum", &report.incident_date.format("%d.%m.%Y").to_string())
        .key_value("Einsatzart", &report.incident_type_label)
        .key_value("Einsatzort", &report.location)
        .key_value("Alarmuhrzeit", &fmt_time(report.alarm_time))
        .key_value("Ausrückzeit",  &fmt_time(report.departure_time))
        .key_value("Ankunftszeit", &fmt_time(report.arrival_time))
        .key_value("Einsatzende",  &fmt_time(report.end_time))
        .key_value("Einsatzleiter", &fmt_opt(report.incident_commander.as_deref()))
        .key_value("Stärke (Führung/Ufl./Mannschaft)",
            &format!("{}/{}/{}", report.strength_leadership, report.strength_sub, report.strength_crew))
        .spacer(4.0);

    if let Some(sit) = report.situation.as_deref().filter(|s| !s.is_empty()) {
        builder = builder.heading("Lage").text_block(sit).spacer(2.0);
    }
    if let Some(meas) = report.measures.as_deref().filter(|s| !s.is_empty()) {
        builder = builder.heading("Maßnahmen").text_block(meas).spacer(2.0);
    }
    if let Some(notes) = report.notes.as_deref().filter(|s| !s.is_empty()) {
        builder = builder.heading("Bemerkungen").text_block(notes).spacer(2.0);
    }

    // Vehicles table
    if !vehicles.is_empty() {
        let v_headers = vec!["Fahrzeug", "Rufname", "Alarmiert", "Ausgerückt", "Am Einsatzort", "Eingerückt", "km"];
        let v_rows: Vec<Vec<String>> = vehicles.iter().map(|v| vec![
            v.vehicle_name.clone(),
            v.callsign.clone().unwrap_or_default(),
            fmt_time(v.alarm_time),
            fmt_time(v.departure_time),
            fmt_time(v.arrival_time),
            fmt_time(v.return_time),
            v.km_driven.map(|k| k.to_string()).unwrap_or_default(),
        ]).collect();
        let v_widths = vec![40.0, 25.0, 20.0, 20.0, 20.0, 20.0, 15.0];
        builder = builder.heading("Fahrzeuge").table(v_headers, v_rows, v_widths).spacer(2.0);
    }

    // Personnel table
    if !personnel.is_empty() {
        let p_headers = vec!["Name", "Funktion", "Dienstgrad", "Von", "Bis"];
        let p_rows: Vec<Vec<String>> = personnel.iter().map(|p| vec![
            p.display_name.clone(),
            p.function.clone().unwrap_or_default(),
            p.role_name.clone().unwrap_or_default(),
            fmt_time(p.entry_time),
            fmt_time(p.exit_time),
        ]).collect();
        let p_widths = vec![55.0, 40.0, 40.0, 17.0, 17.0];
        builder = builder.heading("Eingesetztes Personal").table(p_headers, p_rows, p_widths);
    }

    let pdf_bytes = builder.build(&font_bytes)
        .map_err(|e| AppError::Internal(anyhow::anyhow!("PDF-Generierung: {e}")))?;

    let filename = format!(
        "Einsatzbericht_{}.pdf",
        report.incident_number.as_deref().unwrap_or("export")
            .replace('/', "-")
    );
...
pub fn router(state: AppState) -> Router<AppState> {
    ...
    let write_routes = Router::new()
        ...
        .route("/:id/personal/:pid",        put(update_incident_personnel).delete(remove_incident_personnel))
        .route("/:id/anhaenge",             post(upload_attachment.layer(DefaultBodyLimit::max(20 * 1024 * 1024))))
    ...
    let read_routes = Router::new()
        ...
        .route("/stats/export",                 get(export_stats))
        ...
        .route("/:id/pdf",                      get(export_incident_pdf))
        .route_layer(middleware::from_fn_with_state(
